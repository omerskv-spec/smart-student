...
function buildHebrewSummary(assignments: ClassroomAssignment[]): string {
    if (assignments.length === 0) return 'No active tasks.';
    ...
    const lines = sorted.slice(0, 10).map((a) => '- ' + a.courseName + ': ' + a.title + ' (due ' + formatDate(a.dueDate) + ')');
...
export async function GET(request: NextRequest) {
    ...
    const { data: cache } = await supabase.from('classroom_cache').select('*').eq('user_id', auth.uid).single();
    ...
    try {
          ...
          if (!clientId || !clientSecret) {
                  const emptyData: ClassroomData = { courses: [], assignments: [], lastUpdated: new Date().toISOString(), summary: 'Google Classroom not connected' };
                  return NextResponse.json(emptyData);
          }
          const { data: userData } = await supabase.from('users').select('*').eq('id', auth.uid).single();
          if (!userData) return NextResponse.json({ error: 'User not found' }, { status: 404 });
          const coursesRes = await fetch('https://classroom.googleapis.com/v1/courses?courseStates=ACTIVE&pageSize=6', { headers: { Authorization: 'Bearer ' + auth.token } });
          if (!coursesRes.ok) {
                  const emptyData: ClassroomData = { courses: [], assignments: [], lastUpdated: new Date().toISOString(), summary: 'Cannot access Google Classroom' };
          ...
          for (const course of courses.slice(0, 6)) {
                  const workRes = await fetch('https://classroom.googleapis.com/v1/courses/' + course.id + '/courseWork?pageSize=8&orderBy=dueDate%20desc', { headers: { Authorization: 'Bearer ' + auth.token } });
                  if (workRes.ok) {
                            const workData = await workRes.json();
                            const assignments = (workData.courseWork || []).map((w: ClassroomAssignment) => ({ ...w, courseName: course.name }));
                            allAssignments.push(...assignments);
                  }
          }
          const data: ClassroomData = { courses, assignments: allAssignments, lastUpdated: new Date().toISOString(), summary: buildHebrewSummary(allAssignments) };
          ...
          const errorData: ClassroomData = { courses: [], assignments: [], lastUpdated: new Date().toISOString(), summary: 'Error loading Classroom data' };
