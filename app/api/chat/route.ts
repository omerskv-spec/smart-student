...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Fetch user first (uid must be resolved before student_files query)
    ...
    // Fetch classroom cache and student files in parallel
    const [{ data: cache }, { data: studentFiles, error: filesError }] = await Promise.all([
    ...
    if (filesError) console.error('student_files error:', filesError);
    ...
    console.log(`uid=${uid} | files=${studentFiles?.length ?? 0} | chars=${studentMaterials.length}`);
