...
export function MessageBubble({ message, avatarUrl }: Props) {
  ...
  return (
        <div className={`flex gap-3 ${isBot ? '' : 'flex-row-reverse'} max-w-[80%] ${isBot ? '' : 'self-end'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0 mt-1 ${isBot ? 'bg-purple-100 text-purple-700' : 'bg-purple-600 text-white'}`}>
                  {isBot ? '*' : avatarUrl ? <img src={avatarUrl} alt="" className="w-full h-full object-cover" /> : 'x'}
                </div>div>
                <div>
                        <div className={`px-4 py-3 text-sm leading-relaxed ${isBot ? 'bg-white border border-[var(--border)] rounded-[4px_18px_18px_18px] text-[var(--text)]' : 'bg-purple-600 text-white rounded-[18px_18px_4px_18px]'}`}>
                          {isBot ? (
                      <>
                        {badge && (
                                        <div style={{
                        ...
                        <p className={`text-[10px] text-[var(--muted)] mt-1 ${isBot ? 'text-right' : 'text-left'}`}>{time}</p>p>
                </div>div>
        </div>div>
      );
}</></div>
