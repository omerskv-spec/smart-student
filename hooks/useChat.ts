...
import type { Message, Conversation, ChatResponse } from '@/types';
...
export function useChat(
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        const payload = { message: content, conversationId: state.currentConversationId };
        let result: ChatResponse;

        try {
          result = await postChat(payload, idToken, abortRef.current.signal);
        } catch (err: unknown) {
          const status = (err as { status?: number }).status;
          if (status === 401 && firebaseUser) {
            // Token expired — refresh and retry once
            const freshToken = await firebaseUser.getIdToken(true);
            result = await postChat(payload, freshToken, abortRef.current.signal);
          } else {
            throw err;
          }
        }
    ...
    [idToken, state.sending, state.currentConversationId, userId, firebaseUser]
  );
